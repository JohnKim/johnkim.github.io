...
description: 
   - 'Feature based structure vs. Screen based structure.
   - 프로젝트를 시작하면 초기에 디렉토리 구조를 어떤 기준으로 설계해야 할지 고민하게 된다. Feature 중심의 구조와 화면 UI 기준의 구조가 무엇이고, 어떤 상황에 무엇이 적합한지 고민해보았다.'
